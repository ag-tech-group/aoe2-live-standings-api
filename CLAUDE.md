...
**Standings freeze at window end** (`_frozen_peak_by_profile`, `app/routers/tournaments.py`): once `grand_finals_date` passes, `/standings`, `/teams/standings`, and the summary peak card rank on (and surface) the **as-of-window-end** peak — the max `player_rating_snapshots` observation at-or-before the bound, falling back to live `max_rating` when a pair has no snapshots (provably unchanged: the recorder self-seeds a baseline on first observation), clamped to live like `/standings/history`. The roster keeps laddering after a race ends and playoff seeding derives from the final table, so a post-window all-time high must not reorder it. `current_rating` stays live. Mid-event the helper returns `None` and the hot path is untouched.
...
**Tournament data window**: `start_date` / `end_date` on `Tournament` bound the per-tournament `tournament_record` aggregation. A null bound is treated as open. `end_date` is mid-rename from `grand_finals_date` (expand phase, migration `61eb825d1551`): the old name dated from #76, when the countdown target and the window end coincided — the Gauntlet's playoffs proved they don't (rated window closes at race end; the real grand-finals date is display schedule → `presentation` bag). During the transition every write path sets BOTH columns, reads return both, a body where they disagree 422s, and internal window filters still read `grand_finals_date` until the post-event contract phase drops it. **Never** PATCH the window end to the actual grand-finals date to "fix" a hero countdown — that pours post-race ladder games into the record and peaks.
