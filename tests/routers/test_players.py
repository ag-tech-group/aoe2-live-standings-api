"""GET /v1/players and GET /v1/players/{profile_id}."""
...
class TestListPlayers:
    ...
    async def test_returns_players_with_ratings_sorted_alphabetically(
        ...
        # Each player carries its own ratings list.
    ...
    async def test_leaderboard_id_filters_ratings_but_keeps_player(
        ...
        response = await client.get("/v1/players", params={"leaderboard_id": 3})
    ...
    async def test_player_included_even_when_no_ratings_match_filter(
        ...
        response = await client.get("/v1/players", params={"leaderboard_id": 99})
        items = response.json()["items"]
...
class TestGetPlayer:
    async def test_unknown_profile_returns_404(self, client: AsyncClient):
        response = await client.get("/v1/players/999999")
    ...
    async def test_returns_player_with_ratings_and_recent_matches(
        ...
        response = await client.get("/v1/players/1")
        payload = response.json()
        ...
        # Each embedded match carries its own players list.
    ...
    async def test_match_limit_caps_recent_matches(
        ...
        response = await client.get("/v1/players/1", params={"match_limit": 2})
        assert len(response.json()["recent_matches"]) == 2

    async def test_match_limit_out_of_range_returns_422(self, client: AsyncClient):
        response = await client.get("/v1/players/1", params={"match_limit": 999})
