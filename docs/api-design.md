...
| `GET` | `/v1/matches` | recent matches involving any tracked player | `?profile_id=N`, `?leaderboard_id=N`, `?state=in_progress\|completed`, `?limit=20`, `?before=<match_id>` for pagination |
...
- Matches use **cursor**-style: `?before=<match_id>&limit=20`, returns `{ items, next_cursor }`. Match IDs are monotonic.
- Standings and players are **bounded** (small N) and return everything; no pagination.
