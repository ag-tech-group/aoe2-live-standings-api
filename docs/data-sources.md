...
- Rotation-follow-up gotcha (#269, June 2026 TG rotation): aoe2insights' own map table lags brand-new maps — its match pages render "Unknown" for them (seen with the new EW pool's `11022`/`11017`, upstream-hinted as Thames/Crownwood). Such ids stay unverifiable until aoe2insights catches up; leave them on the `mapname` fallback rather than adding a guessed entry, and re-check after their next data update. Custom lobbies hosting a *standard* map carry that map's positive locstring while upstream `mapname` is the literal `"my map"` — so verified entries also fix custom-game labels, and ranked-pool coverage can be probed from top-ladder histories alone (matchtype 6 = 1v1 RM, 7/8/9 = team RM, 26/27 = EW).
...
- **In-progress match visibility on `getRecentMatchHistory`.** ~~Does a match appear here with `completiontime=0` (or absent) while still ongoing, or only after it ends?~~ **Answered 2026-06-10 (during the #267 investigation): only after it ends.** Probed 40 top ladder players at EU prime time (4,048 matches): zero in-progress rows. Combined with ranked automatch never appearing on `findAdvertisements` (customs only), live ranked detection is structurally impossible on the public community surfaces — sites that show live ranked games run dedicated game-protocol infra. `in_match` can only light for custom lobbies.
